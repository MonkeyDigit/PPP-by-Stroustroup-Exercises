...
public:
	...
	Token get(ifstream& ifs);					//function to get tokens
	void unget(Token t) { buffer = t; full = true; }		//renamed putback function: puts tokens back in the input stream via the buffer

	void ignore(char, ifstream& ifs);					//ignores all characters before a ';' in case of error, and continue calculating
...
void clean_up_mess(Token_stream& ts, ifstream& ifs);			//To continue calculating after an error
void calculate(Token_stream& ts, ifstream& ifs);				//Handle Input and Output of statements
double statement(Token_stream& ts, ifstream& ifs);				//Handle expression and declarations
double expression(Token_stream& ts, ifstream& ifs);			//Handle set of operands [+;-]
double term(Token_stream& ts, ifstream& ifs);					//Handle set of operands [*;/]
double primary(Token_stream& ts, ifstream& ifs);				//Handle numbers, parenthesis and variables
double declaration(Token_stream& ts, ifstream& ifs);			//Handle declarations
double assignment(Token_stream& ts, ifstream& ifs);			//Handle assignments
