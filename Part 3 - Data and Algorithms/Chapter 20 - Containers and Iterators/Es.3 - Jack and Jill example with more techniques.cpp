...
double* high_arr(double arr[],const int n)
{
	double h = -1;
	double* high;
	for(int i=0;i<n;++i)
		if (h<arr[i]) { high = &arr[i]; h = arr[i]; }
	
	return high;
}

double* high_vec(vector<double>& vec)
{
	double h = -1;
	double* high;
	for(double& d : vec)
...
void fct()
{
	...
	double* jack_high;
	//jack_high=high_ptr(jack_data,jack_data+jack_count);
	jack_high=high_arr(jack_data,jack_count);
	
	vector<double>& v = *jill_data;
	
	double* jill_high;
	//jill_high=high_ptr(&v[0],&v[0]+v.size());
	jill_high=high_vec(v);
