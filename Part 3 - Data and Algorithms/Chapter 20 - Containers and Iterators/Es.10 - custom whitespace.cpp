...
bool is_whitespace(const char ch, const string wspace)
{
	for(const char ws : wspace)
	{
...
int word_count(Document& doc, string wspace)
{
	int count{0};
	bool word=false;
	for(auto p : doc)
	{
		if(word && is_whitespace(p,wspace))
		{
			++count;
			word=false;
		}
		else if(!is_whitespace(p,wspace))
		{
			word=true;
		}
	}
	
	if(word) ++count; //this is needed because when p (in the for loop) gets to the last character and the loop ends,
					  //we can't count it as a word because to do so the current character must be a whitespace
	return count;
}
int main()
try
{
	ifstream ifs{"es10doc.txt"};
	
	Document doc;
	
	ifs>>doc;
	
	cout << "word count: " << word_count(doc, "!?") << '\n';
	return 0;
}
catch(exception & e)
{
